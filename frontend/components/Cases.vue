...
<template>
  <div class="swiper w-full h-full max-h-screen">
    <div class="swiper-wrapper">
      <div
        v-for="item in fakeContentStore.cases"
        :key="item.id"
        @click="fakeContentStore.selectCase(item)"
        class="swiper-slide md:!h-auto"
      >
        <div
          class="grid gap-3 py-4 rounded-sm duration-300 cursor-pointer md:hover:bg-slate-500/20 md:px-6"
        >
          <h2
            v-if="item.id === 1"
