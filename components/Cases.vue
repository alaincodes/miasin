...
const staticContent = [
  ...
  },
  {
    date: '15/12/24',
    time: '11h04',
    message: 'Quisquam, tempora facilis rem deserunt doloribus natus!',
    user: 'The accused 2',
    judgement: 'Innocent',
  },
  {
  ...
  <div class="grid grid-cols-1 gap-9 content-start md:h-full md:overflow-y-auto">
    ...
    <ul class="flex flex-col divide-y divide-slate-900">
      <li v-for="(item, index) in staticContent" :key="index">
        <div class="prose max-w-full w-full py-4 rounded-sm duration-300 cursor-pointer hover:bg-slate-500/20 md:p-6">
          ...
          <p>Verdict: <strong>{{ item.user }}</strong> is
            <span class="font-bold uppercase" :class="item.judgement === 'Guilty' ? 'text-red-600' : 'text-green-600'">
              {{ item.judgement }}
            </span>
          </p>
        </div>
